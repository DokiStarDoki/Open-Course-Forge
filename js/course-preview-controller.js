 ...
 * Course Forge MVP - Course Preview Controller (UPDATED FOR VERTICAL SCROLL)
...
class CoursePreviewController {
  ...
  /**
   * Generate HTML for vertical scroll course preview (NEW METHOD)
   */
      ...
      .map((chunk, index) => {
        ...
        return `
          <section class="course-section" id="section-${index}" data-section="${index}">
            ...
            ${index < chunks.length - 1 ? '<div class="section-divider"></div>' : ''}
          </section>
        `;
      })
      .join("");
      ...
      <body>
        ...
        <header class="course-header-fixed">
          <div class="header-content">
            <h1 class="course-title-small">${this.escapeHtml(courseConfig.title)}</h1>
        ...
        <div class="course-container">
          ...
          <header class="course-hero">
            <div class="hero-content">
              <h1 class="course-title">${this.escapeHtml(courseConfig.title)}</h1>
          ...
          <footer class="course-completion">
            <div class="completion-content">
              ...
              <p>Congratulations on completing this course. You've covered all ${chunks.length} sections.</p>
        ...
        <nav class="floating-nav" id="floatingNav">
          ...
          <div class="nav-sections">
            ${chunks.map((_, index) => `
              <button class="nav-dot" onclick="scrollToSection(${index})" title="Section ${index + 1}"></button>
            `).join('')}
          </div>
  ...
  renderCourseMetadata(courseConfig) {
    ...
    if (courseConfig.targetAudience) {
      metadata.push(`<span><i data-lucide="users"></i> ${this.escapeHtml(courseConfig.targetAudience)}</span>`);
    }
    
    if (courseConfig.estimatedDuration) {
      metadata.push(`<span><i data-lucide="clock"></i> ${this.escapeHtml(courseConfig.estimatedDuration)}</span>`);
    }
  ...
  renderLearningObjectives(objectives) {
    ...
    return `
      <div class="learning-objectives">
        ...
        <ul>
          ${objectives
            .map((obj) => `<li><i data-lucide="check"></i>${this.escapeHtml(obj)}</li>`)
            .join("")}
        </ul>
      </div>
    `;
  ...
  /**
   * Get CSS styles for vertical scroll layout (NEW METHOD)
   */
  ...
  /**
   * Get JavaScript for vertical scroll functionality (NEW METHOD)
   */
      ...
      function loadProgress() {
        try {
          ...
          if (saved) {
            ...
            setTimeout(() => {
              window.scrollTo({ top: progress.scrollPosition || 0, behavior: 'smooth' });
            }, 1000);
          }
        } catch (e) {
          // Ignore localStorage errors
  ...
  // ... (rest of the methods remain the same: renderBasicSlidePreview, getSlideTypeLabel, escapeHtml, etc.)
